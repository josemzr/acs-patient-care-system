...
export default (props: ConfigurationScreenProps): JSX.Element => {
  ...
  useEffect(() => {
    // Use prefilled display name or get from local storage
    const defaultDisplayName =
      prefilledDisplayName || (localStorageAvailable ? getDisplayNameFromLocalStorage() : null);
  ...
  const setupAndJoinChatThreadWithNewUser = useCallback(() => {
    const internalSetupAndJoinChatThread = async (): Promise<void> => {
      const threadId = prefilledThreadId || getExistingThreadIdFromURL();
  ...
  }, [configurationScreenState, joinChatThreadWithExistingUser, prefilledThreadId]);
    ...
    return (
      ...
      >
        <Stack horizontalAlign="center" tokens={leftPreviewContainerStackTokens} className={leftPreviewContainerStyle}>
          ...
          {conversationTitle && (
            <Text variant="mediumPlus" style={{ marginBottom: '10px', color: theme.palette.themePrimary }}>
              Joining: {conversationTitle}
            </Text>
          )}
